...
class TestFileConfig(TestCase):
    ...
    def test_get(t) -> None:
        ...
        with t.subTest("single key"):
            t.assertEqual(
                conf.get("bat.key"), EXAMPLE_CONFIG_DICT["example"]["bat"]["key"]
            )
    ...
    def test_default_file(t):
        ...
        t.m_open.assert_called_with(t.Path(os.getcwd() + DEFAULT_FILE))

    def test_loads_given_config_file(t):
        with patch("builtins.open", t.m_open):
            FileConfig("./test_example.config.yaml", config_env="example")
    ...
    def test__getitem__(t):
        ...
        with t.subTest("dot notation key path"):
            t.assertEqual(config_file["bat.remote_host.api_key"], "example_api_key")
        with t.subTest("__getitem__ chain"):
            t.assertEqual(
                config_file["bat"]["remote_host"]["url"], "https://api-example.host.io/"
            )
...
class Test_load_config_file(TestCase):
    ...
    def test_config_file_env_variable(t):
        ...
        t.assertEqual(example_env["remote_host"]["url"], "https://api-example.host.io/")

    @patch.dict(f"{SRC}.os.environ", {}, clear=True)
    def test_default_config_file(t):
        Path.is_file = create_autospec(Path.is_file, spec_set=True, return_value=True)
        t.m_open = mock_open(read_data=EXAMPLE_CONFIG_YAML)

        with patch("builtins.open", t.m_open):
            CONF = load_config_file()

        t.m_open.assert_called_with(Path(os.getcwd() + "/config.yaml"))
        example_env = CONF["example"]["bat"]
        t.assertEqual(example_env["remote_host"]["api_key"], "example_api_key")
        t.assertEqual(example_env["remote_host"]["url"], "https://api-example.host.io/")
