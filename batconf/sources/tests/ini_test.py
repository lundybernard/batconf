...
CONFIG_PARSER_ENVS.read_string(INI_EV_STR)
...
class IniConfigTests(TestCase):
    ...
    def test_get_methods_for_file_formats(t) -> None:
        ...
        v: Final = 'flat'
        _ = IniConfig(file_path='na', file_format=v)
...
class GetConfigFunctionsTests(TestCase):
    ...
    def test__get_flat(t):
        ...
        * a default 'root' section is injected into the ConfigParser
        ...
        This is valid flat formatted config.ini contence:
            section.key=value
            section.key2=value2
...
"""
    def test__load_ini_with_environments(t):
        for (
            when_missing,
            missing_file_handler,
        ) in _missing_file_handlers.items():
            with t.subTest(f'{when_missing=}'):
                ret = _load_ini(
                    file_path=t.file_path,
                    file_format='environments',
                    when_missing=when_missing,
                )
                missing_file_handler.assert_called_with(
                    file_path=t.file_path,
                    loader_fn=_load_ini_file,
                    empty_fallback=EmptyConfigParser,
                )
                t.assertIs(t.load_file_warn_when_mising.return_value, ret)
                t.load_file_warn_when_mising.reset_mock()

        with t.subTest('file_format="flat"'):
            ret = _load_ini(
                file_path=t.file_path,
                file_format='flat',
                when_missing='warn',
            )
            t.load_file_warn_when_mising.assert_called_with(
                file_path=t.file_path,
                loader_fn=_load_ini_file_flat,
                empty_fallback=EmptyConfigParser,
            )
            t.assertIs(t.load_file_warn_when_mising.return_value, ret)

    def test__load_ini_ignore(t):
        for file_format in ('sections', 'environments'):
            with t.subTest(f'{file_format=}'):
                ret = _load_ini(
                    file_path=t.file_path,
                    file_format=file_format,
                    when_missing='ignore',
                ...
                load_file_ignore_when_missing.reset_mock()
        ...
        with t.subTest('file_format="flat" error handling'):
            raise NotImplementedError

    @patch(f'{SRC}._load_ini_file_flat', autospec=True)
    @patch(f'{SRC}._load_ini_file', autospec=True)
    def test__load_ini_error(
        t,
        _load_ini_file: Mock,
        _load_ini_file_flat: Mock,
    ):
        for file_format in ('sections', 'environments'):
            with t.subTest(f'{file_format=}'):
                ret = _load_ini(
        ...
        with t.subTest(f'{file_format=}'):
            ...
            _load_ini_file_flat.assert_called_with(file_path=t.file_path)
            _load_ini_file_flat.reset_mock()

        with t.subTest('file_format="flat" error handling'):
            _load_ini_file_flat.side_effect = FileNotFoundError
            with t.assertRaises(FileNotFoundError) as err:
                ...
                _load_ini_file_flat.assert_called_with(file_path='flat')
"""
