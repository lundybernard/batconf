...
def argparser(cfg_path='root'):
    p = ArgumentParser()
    p.add_argument('-a', dest=f'{cfg_path}.a')
...
def command_cli(cfg_path: str = 'command'):
    p = ArgumentParser()
...
class NamespaceSourceTests(TestCase):
    def test_(t) -> None:
        ...
        t.assertEqual(getattr(namespace, 'root.a'), 'value_a')
        ...
        t.assertEqual(src.get('root.a'), 'value_a')
