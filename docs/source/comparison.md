...
### Config sources and layering

| Tool | Config sources | Layering / precedence |
|---|---|---|
| **BatConf** | CLI args, env vars, YAML/TOML/INI files, dataclass defaults; custom sources via protocol | Default: CLI > env > files > defaults; order customizable via `SourceList` |
| [pydantic-settings](https://docs.pydantic.dev/latest/concepts/pydantic_settings/) | Env vars, JSON/YAML/TOML/`pyproject.toml`, secrets files, cloud secret managers | CLI (when enabled) > init args > env > `.env` > defaults |
| [environs](https://github.com/sloria/environs) | Env vars (Twelve-Factor focus) | Env-var oriented (single tier) |
| [python-decouple](https://github.com/HBNetwork/python-decouple) | Env vars, `.ini`/`.env` files, defaults | Env > file > default |
| [Dynaconf](https://www.dynaconf.com/) | TOML/YAML/JSON/INI/Python/`.env`, env vars | Multi-environment profiles + env switcher |
| [python-dotenv](https://github.com/theskumar/python-dotenv) | `.env` files only | None — loads into `os.environ` or a dict; caller decides layering |
| [Hydra](https://hydra.cc/docs/intro/) / [OmegaConf](https://omegaconf.readthedocs.io/) | YAML config files + command line; OmegaConf also merges dataclasses | Hierarchical composition + CLI `key=value` overrides |
| [configparser](https://docs.python.org/3/library/configparser.html) (stdlib) | INI files | Multiple files read sequentially; later overrides earlier |

### Validation and CLI support

| Tool | Validation / typing | CLI support |
|---|---|---|
| **BatConf** | Dataclasses + type hints; validation deliberately out of scope — bring your own | Explicit: wraps your app's own argparse `Namespace` as a source |
| pydantic-settings | pydantic model validation | Built-in CLI parsing with subcommands and help text |
| environs | marshmallow validation + rich type casting | None |
| python-decouple | Explicit `cast` callable; no schema validation | None |
| Dynaconf | `Validator` objects / TOML schemas; token auto-cast | Management CLI (init/list/write/validate/export) |
| python-dotenv | None (parser/loader only) | None |
| Hydra / OmegaConf | OmegaConf structured configs (opt-in runtime type safety) | Command-line composition is central; `@hydra.main` owns `argv` |
| configparser (stdlib) | None — values are strings; caller converts | None |
...
and have large communities. If ecosystem size and abundance of
third-party examples are decisive for your team, weigh that honestly —
the mainstream tools win on that axis today. BatConf is a much smaller
project, but it is actively maintained, a 1.0 release is on the
roadmap, and professional support is available through the
